...
"""
# path : root/2_order_service/order_application/order_api/routes.py
import datetime
...
from .order_utils import make_response_409, make_general_response, check_add_order_keys, get_add_order_params, \
    initialize_order
    ...
    try:
        if request.method == 'GET':
            if "customer-id" in request.args and 'order-id' in request.args:
                ...
                customer_orders = OrdersOrm.query.filter_by(c_id=customer_id, o_id=order_id)
    ...
    if request.method == 'POST':
        try:
            ...
            if is_params_valid:
                ...
                order = OrdersOrm()
                # fill order object
                order = initialize_order(order, form_data["o-id"][0]['o-id'], c_id, o_address, o_city, o_postal_code,
                                         o_phone_number, o_status, o_date)
                # add to session
